...
import { Router as MemoryRouter, Route, Routes, memoryIntegration } from '@solidjs/router';
...
export const CreateProduct: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const firstRow = canvasElement.querySelector('tbody > tr.hope-tr')!;
...
export const UpdateProduct: Story = {
  ...
  play: async ({ canvasElement }) => {
    await sleep(SLEEP_TIME);
    let firstRow = canvasElement.querySelector('tbody > tr.hope-tr')!;
    ...
    firstRow = canvasElement.querySelector('tbody > tr.hope-tr')!;
    const firstRowData = Array.from(firstRow.querySelectorAll('td')).map((td) => td.innerText);
...
export const RemoveProduct: Story = {
  ...
  play: async ({ canvasElement }) => {
    await sleep(SLEEP_TIME);
    let firstRow = canvasElement.querySelector('tbody > tr.hope-tr')!;
    const [initialFirstRowId] = Array.from(firstRow.querySelectorAll('td')).map((td) => td.innerText);
    const actionBtns = Array.from(firstRow.querySelectorAll('button'));
    ...
    firstRow = canvasElement.querySelector('tbody > tr.hope-tr')!;
    const [currentFirstRowId] = Array.from(firstRow.querySelectorAll('td')).map((td) => td.innerText);
